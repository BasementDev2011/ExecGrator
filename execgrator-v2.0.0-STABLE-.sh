...
then
{
    ...
    chmod +x "$HOME/Applications/$executable_name_no_extension/$executable_name"
cat <<EOF > "$HOME/Applications/$executable_name_no_extension/$executable_name_no_extension.desktop"
[Desktop Entry]
...
then
    ...
    relative_executable="${executable_location#$folder_location/}"
    read -p "Provide Application category (Game,Accessories,Development,Internet, etc..): " executable_category
    read -p "Provide Application icon location (optional, press ENTER to skip): " executable_icon
    cp -R "$folder_location" "$HOME/Applications"
cat <<EOF > "$HOME/Applications/$folder_name/$folder_name.desktop"
[Desktop Entry]
Name=$folder_name
Exec=$HOME/Applications/$folder_name/$relative_executable
Type=Application
Categories=$executable_category;
Terminal=false
Icon=$executable_icon
EOF
    cp "$HOME/Applications/$folder_name/$folder_name.desktop" "$HOME/.local/share/applications/"
